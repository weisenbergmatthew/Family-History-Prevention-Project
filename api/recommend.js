...
- BE CONCISE. Each recommendation is a short, scannable imperative — ideally under 15 words, a single line. State the action and any concrete number/target, and STOP. Do NOT append a rationale clause ("to reduce X risk", "to support healthy blood pressure", "since alcohol is linked to…") — the condition tag already conveys the why. Prefer "≤2,300 mg sodium/day; follow a DASH pattern" over a full explanatory sentence.
- Keep it tight: at most 3 items per category (1-2 if the history is sparse); merge related advice into one line rather than splitting it. Include only the highest-impact items. Do not fabricate exact screening ages you are unsure of — where timing depends on the individual, say "discuss timing with your clinician".
